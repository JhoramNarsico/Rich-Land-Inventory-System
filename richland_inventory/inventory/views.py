...
def get_change_summary(record):
    ...
    for change in delta.changes:
        ...
        if change.field == 'category':
            try:
                old_display_value = Category.objects.get(pk=change.old).name if change.old else 'None'
            except Category.DoesNotExist:
                old_display_value = f"Deleted Category (ID: {change.old})"
            try:
                new_display_value = Category.objects.get(pk=change.new).name if change.new else 'None'
            except Category.DoesNotExist:
...
class ReportingView(LoginRequiredMixin, PermissionRequiredMixin, TemplateView):
    ...
    def export_transactions_pdf(self, request):
        ...
        if form.is_valid():
            ...
            if start_date:
                transactions_base = transactions_base.filter(timestamp__date__gte=start_date)
            if end_date:
                transactions_base = transactions_base.filter(timestamp__date__lte=end_date)

        sales_transactions = transactions_base.filter(transaction_type='OUT', selling_price__isnull=False)
        summary = sales_transactions.aggregate(
            total_revenue=Sum(F('selling_price') * F('quantity')),
            total_items_sold=Sum('quantity')
        )
        top_sellers = sales_transactions.values('product__name').annotate(
            total_quantity_sold=Sum('quantity')
        ).order_by('-total_quantity_sold')[:5]

        context = {
            'transactions': transactions_base.order_by('timestamp'),
        ...
        if not isinstance(pdf, HttpResponse):
...
class ReportsDashboardView(LoginRequiredMixin, PermissionRequiredMixin, TemplateView):
    template_name = 'inventory/reports_dashboard.html'
    permission_required = 'inventory.can_view_reports'
    ...
    def get_context_data(self, **kwargs):
        ...
        # All stock-out (sales) in the year
        ...
        monthly_sales = [0] * 12
        for s in sales:
            month = s.timestamp.month - 1
            monthly_sales[month] += float(s.selling_price * s.quantity)
        context['monthly_sales'] = monthly_sales
        context['months'] = [calendar.month_abbr[i] for i in range(1, 13)]

        # 2. Stock Status Distribution (Corrected Keys)
        ...
        # 3. Top 5 Selling Products (Data prepared for Chart.js)
        top_sellers_qs = sales.values('product__name') \
            .annotate(total=Sum('quantity')) \
            .order_by('-total')[:5]
        # Prepare separate lists for labels and data
        context['top_sellers_labels'] = [item['product__name'] for item in top_sellers_qs]
        context['top_sellers_data'] = [item['total'] for item in top_sellers_qs]

        # 4. Inventory Value vs Target (Data prepared for Chart.js)
        total_value = Product.objects.aggregate(
            total=Sum(F('price') * F('quantity'))
        )['total'] or 0
        target_value = getattr(settings, 'TARGET_INVENTORY_VALUE', 500_000)
        
        context['inventory_value'] = float(total_value)
        context['target_value'] = float(target_value)
        # Calculate the remaining value and add it to the context
        remaining = target_value - total_value
        context['inventory_value_remaining'] = max(0, float(remaining))


        # 5. Recent Transactions
        context['recent_transactions'] = sales.select_related('product', 'user').order_by('-timestamp')[:10]

        # 6. Category Health
        context['category_limits'] = Category.objects.annotate(
            total_products=Count('products'),
            low_stock=Count('products', filter=Q(products__quantity__lte=F('products__reorder_level'))),
            out_of_stock=Count('products', filter=Q(products__quantity=0)),
        ).values('name', 'total_products', 'low_stock', 'out_of_stock')

        # Year selector
        context['current_year'] = year
        context['available_years'] = list(range(timezone.now().year, 2020, -1))
