...
class ProductListView(LoginRequiredMixin, PermissionRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if form.is_valid():
            ...
            if query: queryset = queryset.filter(Q(name__icontains=query) | Q(sku__icontains=query))
            category = form.cleaned_data.get('category')
            if category: queryset = queryset.filter(category=category)
            stock_status = form.cleaned_data.get('stock_status')
            if stock_status == 'in_stock': queryset = queryset.filter(quantity__gt=10)
            elif stock_status == 'low_stock': queryset = queryset.filter(quantity__gt=0, quantity__lte=10)
            elif stock_status == 'out_of_stock': queryset = queryset.filter(quantity=0)
            sort_by = form.cleaned_data.get('sort_by')
            if sort_by: queryset = queryset.order_by(sort_by)
            else: queryset = queryset.order_by('-date_created')
        return queryset
...
class ProductCreateView(LoginRequiredMixin, PermissionRequiredMixin, SuccessMessageMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        if initial_quantity > 0:
            StockTransaction.objects.create(product=self.object, transaction_type='IN', quantity=initial_quantity, user=self.request.user, notes='Initial stock on product creation.')
        return response
...
class ProductUpdateView(LoginRequiredMixin, PermissionRequiredMixin, SuccessMessageMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        if quantity_difference > 0:
            StockTransaction.objects.create(product=self.object, transaction_type='IN', quantity=quantity_difference, user=self.request.user, notes='Manual quantity update from product edit form.')
        elif quantity_difference < 0:
            StockTransaction.objects.create(product=self.object, transaction_type='OUT', quantity=abs(quantity_difference), user=self.request.user, notes='Manual quantity update from product edit form.')
        return response
...
# --- DELETED ProductTransactionCreateView ---
# The entire ProductTransactionCreateView class has been removed.
...
class ReportingView(LoginRequiredMixin, PermissionRequiredMixin, TemplateView):
    ...
    def export_transactions_pdf(self, request):
        ...
        if form.is_valid():
            ...
            if start_date: transactions = transactions.filter(timestamp__date__gte=start_date)
            if end_date: transactions = transactions.filter(timestamp__date__lte=end_date)
